...
function showScreen(id) {
  ['screen-login','screen-setup','screen-session','screen-done',
   'screen-user-mgmt','screen-edit-user-cfg'].forEach(s => {
...
function initApp(session) {
  // Update user bar — show full name if the user record has one
  const userRecord  = Auth.getUsers().find(u => u.username === session.username);
  const displayName = (userRecord && userRecord.fullName)
    ? `${userRecord.fullName} (${session.username})`
    : session.username;
  document.getElementById('user-bar-name').textContent = displayName;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('btn-use-default').addEventListener('click', () => {
    fetch('exercise-scripts/default-config.cfg')
      .then(r => r.text())
      .then(text => loadCfg(text, 'built-in sample'))
      .catch(() => alert('Could not load the built-in exercises. Please try uploading a .cfg file instead.'));
  });
